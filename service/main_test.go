...
/*
pipeline test. It checks that values processed in parallel.
The results of the jobs should not be stored inside
job functions and should be passed as they come to the next
job function.
*/
...
func TestSigner(t *testing.T) {
	...
	fmt.Println("Test result is ok. Checking time...")

	if end > expectedTime {
		t.Errorf("execition too long\nGot: %s\nExpected: <%s", end, time.Second*3)
	}

	// 8 because 2 in SingleHash and 6 in MultiHash
